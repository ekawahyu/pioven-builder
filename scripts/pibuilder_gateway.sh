...
if ! (grep -q "./scripts/first_run_gateway.sh" "${ROOT_DIR}/etc/rc.local"); then
#  sed -i '$ d' "${ROOT_DIR}/etc/rc.local"
#  printf "if [ -f /first_run_gateway.sh ]; then\n  printf 'Setting up the Pi'\n  sh /first_run_gateway.sh\n  reboot\n\n  printf 'Pi setup'\nfi\n\nexit 0\n" >> "${ROOT_DIR}/etc/rc.local"
sudo sed -i '/exit 0/i \/first_run_gateway.sh\n' "${ROOT_DIR}/etc/rc.local"
fi

cp ./scripts/first_run_gateway.sh "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_HOSTNAME%/$PI_HOSTNAME/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_USERNAME%/$PI_USERNAME/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_PASSWORD%/$PI_PASSWORD/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_WIFI_SSID%/$PI_WIFI_SSID/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_MAILGUN_API_KEY%/$PI_MAILGUN_API_KEY/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_MAILGUN_DOMAIN%/$PI_MAILGUN_DOMAIN/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_EMAIL_ADDRESS%/$PI_EMAIL_ADDRESS/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_INSTALL_DOCKER%/$PI_INSTALL_DOCKER/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_GPU_MEMORY%/$PI_GPU_MEMORY/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_IP_ADDRESS_RANGE_START%/$PI_IP_ADDRESS_RANGE_START/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_IP_ADDRESS_RANGE_END%/$PI_IP_ADDRESS_RANGE_END/g" "${ROOT_DIR}/first_run_gateway.sh"
sed -i "s/%PI_DNS_ADDRESS%/$PI_DNS_ADDRESS/g" "${ROOT_DIR}/first_run_gateway.sh"
